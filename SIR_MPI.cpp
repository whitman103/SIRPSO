...
void FuzzyTree::calculateDeltaMembershipValues() {
    if(delta<delta1) {
        ...
        deltaMembershipValues= {
            .same = (delta2-delta)/(delta2-delta1),
            .near = (delta-delta1)/(delta2-delta1),
            .far = 0
        };
    }
    else if(delta>delta2&&delta<delta3) {
        deltaMembershipValues= {
            .same = 0,
            .near = (delta3-delta)/(delta3-delta2),
            .far = (delta-delta2)/(delta3-delta2)
        };
...
void FuzzyTree::setInertia() {
    inertia=0;
    inertia+=inertiaMap.low*(phiMembershipValues.worse+deltaMembershipValues.same);
    inertia+=inertiaMap.medium*(phiMembershipValues.same+deltaMembershipValues.near);
    inertia+=inertiaMap.high*(phiMembershipValues.better+deltaMembershipValues.far);
    inertia/=2.;
}

void FuzzyTree::setSocial() {
    social=0;
    social+=socialMap.low*(phiMembershipValues.better+deltaMembershipValues.near);
    social+=socialMap.medium*(phiMembershipValues.same+deltaMembershipValues.same);
    social+=socialMap.high*(phiMembershipValues.worse+deltaMembershipValues.far);
    social/=2.;
}

void FuzzyTree::setCognitive() {
    cognitive=0;
    cognitive+=cognitiveMap.low*deltaMembershipValues.far;
    cognitive+=cognitiveMap.medium*(phiMembershipValues.worse+phiMembershipValues.same+deltaMembershipValues.same+deltaMembershipValues.near);
    cognitive+=cognitiveMap.high*(phiMembershipValues.better);
    cognitive/=2.;
}

void FuzzyTree::setL() {
    L=0;
    L+=LMap.low*(phiMembershipValues.same+phiMembershipValues.better+deltaMembershipValues.far);
    L+=LMap.medium*(deltaMembershipValues.same+deltaMembershipValues.near);
    L+=LMap.high*(phiMembershipValues.worse);
    L/=2.;
}

void FuzzyTree::setU() {
    U=0;
    U+=UMap.low*(deltaMembershipValues.same);
    U+=UMap.medium*(phiMembershipValues.better+phiMembershipValues.same+deltaMembershipValues.near);
    U+=UMap.high*(phiMembershipValues.worse+deltaMembershipValues.far);
    U/=2.;
}
...
double calculateMoment(vector<double>& inVector, double order) {
    double outValue(0);
    return accumulate(inVector.begin(),inVector.end(),outValue,[&order](double x,double y) {
        return x+pow(y,order);
    })/(double)inVector.size();
}
