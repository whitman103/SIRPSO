...
int main() {
    ...
    for_each(wMat.begin(), wMat.end(), [&normalizationConst](vector<double>& v) {
        transform(v.begin(), v.end(), v.begin(), bind(multiplies<double>(), std::placeholders::_1, normalizationConst));
    });
    wMat = invertMatrix(wMat);
    double maxValue(1 / findMaxElement(wMat));
    for_each(wMat.begin(), wMat.end(), [&maxValue](vector<double>& v) {
        transform(v.begin(), v.end(), v.begin(), bind(multiplies<double>(), std::placeholders::_1, maxValue));
    });
    for (int i = 0; i < (int)wMat.size(); i++) {
        for (int j = 0; j < (int)wMat[i].size(); j++) {
            cout << wMat[i][j] << " ";
