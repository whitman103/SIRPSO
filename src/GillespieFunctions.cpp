...
tuple<int, double> Gillespie::PerformTimeStep2(vector<int>& specNum) {
    ...
    double a0 = sumupslow(specNum, reactConsts, propCoeffs, propVector);
    if (a0 == 0.) {
        return make_tuple(-1, -5);
    } else {
        return make_tuple(r1choice(a0, r1, propVector), tau(a0, r2));
    }
}
...
int Gillespie::r1choice(double a0, double rand, const vector<double> avector) {
...
double Gillespie::partial_sum(vector<double> v, int index) {
    return std::accumulate(v.begin(), v.begin() + index + 1, 0);
}
...
void Gillespie::specChange(vector<int>& specNum, const int reactChoice, const vector<vector<int> > changeCoeffs) {
...
void Gillespie::initializeReactions(string coeffs, vector < vector < int > >& changeCoeffs, vector < vector < int > >& propCoeffs) {
    ifstream myfile;
